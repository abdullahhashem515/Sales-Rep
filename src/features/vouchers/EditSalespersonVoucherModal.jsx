...
export default function EditSalespersonVoucherModal({ show, onClose, slug }) {
...
        const voucherRes = await get(`admin/payment-vouchers/${slug}`, token);
...
        const [salesRepsRes, currenciesRes] = await Promise.all([
...
        // Set form field values from fetched voucher
...
    return salesReps.map(rep => ({ value: rep.id, label: rep.name }));
  }, [salesReps]);

  const currencyOptions = useMemo(() => {
    return currencies.map(currency => ({ value: currency.id, label: `${currency.name} (${currency.code})` }));
  }, [currencies]);
...
        toast.success('تم تعديل سند السحب بنجاح!');
        onClose(true);
      } else {
...
    return (
      <ModalWrapper show={show} onClose={() => onClose(false)} isVisible={isVisible} title="تعديل سند سحب من المندوب" maxWidth="max-w-xl">
