...
export default function AddSalespersonVoucherModal({ show, onClose }) {
  ...
  // Fetch initial data (sales reps and currencies)
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response?.status === true) {
        toast.success('تم إنشاء سند السحب بنجاح!');
        onClose(true);
      } else {
  ...
  if (loadingInitialData) {
    return (
      <ModalWrapper show={show} onClose={() => onClose(false)} isVisible={isVisible} title="إضافة سند سحب من المندوب" maxWidth="max-w-xl">
