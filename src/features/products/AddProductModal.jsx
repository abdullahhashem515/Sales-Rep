...
export default function AddProductModal({
  ...
  const [selectedPriceCurrencyCode, setSelectedPriceCurrencyCode] =
    useState("");
  ...
  useEffect(() => {
    if (show) {
      ...
      console.log(
        "AddProductModal: Initializing with availableCurrencies (detailed):",
        JSON.stringify(availableCurrencies, null, 2)
      );
    ...
    const fetchCategories = async () => {
      ...
      try {
        ...
        if (Array.isArray(response)) {
          ...
          console.log(
            "AddProductModal: Categories state after fetch (direct array):",
            response
          );
        } else if (response && Array.isArray(response.categories)) {
          setCategories(response.categories);
          console.log(
            "AddProductModal: Categories state after fetch (from object.categories):",
            response.categories
          );
        } else {
          console.warn(
            "AddProductModal: API response for categories is unexpected:",
            response
          );
  ...
  const currentPricesForSelectedCurrency =
    allProductPrices[selectedPriceCurrencyCode] || [];
    ...
    if (currentPricesForSelectedCurrency.length < 5) {
      setAllProductPrices((prevPrices) => ({
        ...
        [selectedPriceCurrencyCode]: [
          ...currentPricesForSelectedCurrency,
          { value: "", type_user: typeUserOptions[0]?.value || "" },
        ],
      }));
    } else {
  ...
  const handleRemovePriceField = (index) => {
    const newPrices = currentPricesForSelectedCurrency.filter(
      (_, i) => i !== index
    );
    setAllProductPrices((prevPrices) => ({
      ...prevPrices,
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(allProductPrices).forEach((currencyCode) => {
      allProductPrices[currencyCode].forEach((priceEntry, index) => {
        ...
        if (!String(priceEntry.value).trim()) {
          currentErrors[
            `priceValue_${currencyCode}_${index}`
          ] = `قيمة السعر للعملة ${currencyCode} مطلوبة.`;
        } else if (isNaN(parseFloat(priceEntry.value))) {
          currentErrors[
            `priceValue_${currencyCode}_${index}`
          ] = `قيمة السعر للعملة ${currencyCode} يجب أن تكون رقماً.`;
        }
        if (
          !priceEntry.type_user ||
          !typeUserOptions.some((opt) => opt.value === priceEntry.type_user)
        ) {
          currentErrors[
            `priceTypeUser_${currencyCode}_${index}`
          ] = `نوع المستخدم مطلوب وصالح لكل سعر.`;
        }
        ...
        if (
          String(priceEntry.value).trim() &&
          !isNaN(parseFloat(priceEntry.value)) &&
          priceEntry.type_user
        ) {
    ...
    try {
      ...
      console.log(
        "AddProductModal: Final payload to API:",
      ...
      if (err.status === 422) {
        ...
        if (backendErrors) {
          if (
            backendErrors.name &&
            backendErrors.name[0].includes("مستخدم بالفعل")
          ) {
            setErrors((prev) => ({
              ...prev,
              productName: "اسم المنتج هذا مستخدم بالفعل. يرجى اختيار اسم آخر.",
            }));
          } else if (
            backendErrors.unit &&
            backendErrors.unit[0].includes("مستخدم بالفعل")
          ) {
            // Specific check for duplicate unit
            setErrors((prev) => ({
              ...prev,
              productUnit:
                "هذه السعة (الوحدة) مستخدمة بالفعل لمنتج آخر. يرجى اختيار سعة مختلفة.",
            }));
          } else if (backendErrors.name) {
            setErrors((prev) => ({
              ...prev,
              productName: backendErrors.name[0],
            }));
          } else if (backendErrors.unit) {
            setErrors((prev) => ({
              ...prev,
              productUnit: backendErrors.unit[0],
            }));
          } else if (backendErrors.category_slug) {
            setErrors((prev) => ({
              ...prev,
              productCategory: backendErrors.category_slug[0],
            }));
          } else if (backendErrors.prices) {
            setErrors((prev) => ({
              ...prev,
              general: "خطأ في التحقق من الأسعار: " + backendErrors.prices[0],
            }));
          } else {
            setErrors({
              general:
                "خطأ في التحقق من البيانات: " + (err.message || "فشل التحقق."),
            });
          }
        } else {
          setErrors({
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-4">
        ...
        <div className="border border-gray-700 p-4 rounded-lg">
          <h4 className="text-lg font-bold mb-3">
            تفاصيل الأسعار (لكل عملة حتى 5 أسعار)
          </h4>
          <FormSelectField
          ...
          <div className="max-h-35 overflow-y-auto pr-2">
            {selectedPriceCurrencyCode &&
              currentPricesForSelectedCurrency.map((priceEntry, index) => (
                <div
                  key={`${selectedPriceCurrencyCode}-${index}`}
                  className="flex flex-col md:flex-row gap-4 mb-3 items-end"
                >
                  <FormInputField
                    label={
                      index === 0 ? `السعر (${selectedPriceCurrencyCode})` : ""
                    }
                    type="text"
                    placeholder="قيمة السعر"
                    value={priceEntry.value}
                    onChange={(e) =>
                      handlePriceFieldChange(index, "value", e.target.value)
                    }
                    error={
                      errors[`priceValue_${selectedPriceCurrencyCode}_${index}`]
                    }
                    className="flex-grow"
                  />
                  <FormSelectField
                    ...
                    onChange={(e) =>
                      handlePriceFieldChange(index, "type_user", e.target.value)
                    }
                    options={typeUserOptions} // Use the updated typeUserOptions
                    error={
                      errors[
                        `priceTypeUser_${selectedPriceCurrencyCode}_${index}`
                      ]
                    }
                    className="flex-grow"
                  />
          ...
          {selectedPriceCurrencyCode &&
            currentPricesForSelectedCurrency.length < 5 && (
              <button
                type="button"
          ...
          {!selectedPriceCurrencyCode && (
            <p className="text-gray-400 text-center">
              يرجى اختيار عملة لإضافة الأسعار.
            </p>
          )}
        </div>
